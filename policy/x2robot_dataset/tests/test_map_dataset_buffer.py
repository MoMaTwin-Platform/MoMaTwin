...
def main(rank, world_size):
    ...
    train_dataset, val_dataset, train_sampler, val_sampler = make_chunk_dataset(
                       dataset_paths,
                       sample_ratio=[0.1]*len(dataset_paths),
                       replacement=False,
                       rank=rank,
                       dp_world_size=world_size,
                       dataset_buffer_size=16, #数据集缓冲区大小
    ...
    train_loader = DataLoader(train_dataset, batch_size=512, sampler=train_sampler, num_workers=10, collate_fn=collate_fn)
