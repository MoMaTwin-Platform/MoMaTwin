...
from lerobot.common.datasets.utils import load_from_disk, hf_transform_to_torch, get_hf_dataset_safe_version, load_dataset
...
def load_hf_dataset(repo_id: str, version: str, root: Path, split: str) -> datasets.Dataset:
    ...
    if root is not None:
        ...
        if split != "train": # may be "train[INT%:]" or "train[:INT%]" or "train[:FLOAT%]" or "train[FLOAT%:]"
            if "%" in split:
                match_from = re.search(r"train\[(\d+(?:\.\d+)?)%:\]", split)
                if match_from:
                    from_frame_index = int(float(match_from.group(1))/100 * len(hf_dataset))

                match_to = re.search(r"train\[:(\d+(?:\.\d+)?)%\]", split)
                if match_to:
                    to_frame_index = int(float(match_to.group(1))/100 * len(hf_dataset))
            else:
                match_from = re.search(r"train\[(\d+(?:\.\d+)?):\]", split)
                if match_from:
