...
class DiffusionUnetTimmPolicy(BaseImagePolicy):
    def __init__(self, 
        ...
        assert len(action_shape) == 1 ,'assert len(action_shape) == 1'
    ...
    def forward(self, batch):
        # normalize input
        assert 'valid_mask' not in batch ,'assert valid_mask not in batch'
        nobs = self.normalizer.normalize(batch['obs'])
        nactions = self.normalizer['action'].normalize(batch['action'])
        
        assert self.obs_as_global_cond, 'assert self.obs_as_global_cond'
        global_cond = self.obs_encoder(nobs)
