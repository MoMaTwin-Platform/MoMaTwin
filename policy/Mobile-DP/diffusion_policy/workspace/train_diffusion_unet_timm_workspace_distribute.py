...
class TrainDiffusionUnetImageWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        for local_epoch_idx in range(cfg.training.num_epochs):
            ...
            tepoch = tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", leave=False, mininterval=cfg.training.tqdm_interval_sec, disable=not accelerator.is_local_main_process)
            ...
            if accelerator.is_main_process and (self.epoch % cfg.training.sample_every) == 0:
                ...
                with torch.no_grad():
                    all_preds = []
                    all_actions = []
                    with tqdm.tqdm(full_val_dataloader, desc=f"predict epoch {self.epoch}", 
                        ...
                        for batch_idx, batch in enumerate(pepoch):
                            ...
                            result = unwrapped_model.predict_action(obs_dict)
