...
# from x2robot_dataset.map_dataset import collate_wrapper
from x2robot_dataset.dataloader import DynamicDataLoader
from x2robot_dataset.common.collate_fn import collate_wrapper
from x2robot_dataset.common.data_preprocessing import _ACTION_KEY_WITH_HAND_MAPPING, _CAM_MAPPING, _CAM_FILE_MAPPING, _CAM_BINOCULAR_FILE_MAPPING, _ACTION_KEY_EE_MAPPING, _CAM_MAPPING_BACKGROUND_AUG, _CAM_MAPPING_LIGHT_AUG
...
class TrainDiffusionUnetImageWorkspace(BaseWorkspace):
    ...
    def train_loop(self, train_num, rank, accelerator, train_dataloader, lr_scheduler, wandb_run, json_logger, cfg, step_log, local_epoch_idx, gradient_accumulation_steps):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            if batch_idx >= train_num-1:
                ...
                break
            # print(f"rank {rank} batch_idx {batch_idx} train_num {train_num}", flush=True)
            ...
            with torch.no_grad():
                ...
                with tqdm.tqdm(total=val_num, desc=f"rank {rank} - predict epoch {self.epoch}", 
                    ...
                    if accelerator.is_main_process and len(all_preds) > 0:
                        ...
                        if self.minmax_range_robot == 'hand_jaka_pos':
                            left_arm_names = ['lx','ly','lz','lx_rot', 'ly_rot', 'lz_rot']
                            right_arm_names = ['rx','ry','rz','rx_rot', 'ry_rot', 'rz_rot']
                            left_hand_names = [f'lhand_{hand_joint_idx}' for hand_joint_idx in range(19)]
                            right_hand_names = [f'rhand_{hand_joint_idx}' for hand_joint_idx in range(19)]
                            action_names = left_arm_names + left_hand_names + right_arm_names + right_hand_names
                        if self.minmax_range_robot == 'hand_jaka_joint':
                            left_arm_names = [f'ljoint_{joint_idx}' for joint_idx in range(6)]
                            right_arm_names = [f'rjoint_{joint_idx}' for joint_idx in range(6)]
                            left_hand_names = [f'lhand_{joint_idx}' for joint_idx in range(19)]
                            right_hand_names = [f'rhand_{joint_idx}' for joint_idx in range(19)]
                            action_names = left_arm_names + left_hand_names + right_arm_names + right_hand_names
    ...
    def run(self):
        ...
        data_aug_type = default(cfg, 'task.data_aug_type', None) # 数据增强类型， background_aug, light_aug
        print(f'data_aug_type:{data_aug_type}', flush=True)
        relative_action = default(cfg, 'task.relative_action', False) # 是否使用相对动作
        ...
        cam_mapping = _CAM_MAPPING # 支持背景增强和光照增强
        if data_aug_type == 'background_aug':
            cam_mapping = _CAM_MAPPING_BACKGROUND_AUG
        elif data_aug_type == 'light_aug':
            cam_mapping = _CAM_MAPPING_LIGHT_AUG
        # 过滤掉不在rgb_keys里的cam
        filter_cam_mapping = {}
