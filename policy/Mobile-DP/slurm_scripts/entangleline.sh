#!/bin/bash
#SBATCH --job-name=entangle_line_threefork
#SBATCH --nodes=1
...
#SBATCH -o /x2robot/liuxingchen/exp/%x-%j.log

export HYDRA_FULL_ERROR=1
export TOKENIZERS_PARALLELISM=false
# wandb setting
export WANDB_ENTITY='x2robot_external_taikang'
...
MASTER_PORT=10232 # use 5 digits ports

NNODES=$SLURM_NNODES
all_gpus=$((SLURM_NNODES * GPUS_PER_NODE))
echo "all_gpus: $all_gpus"
# no config_file for ddp
export NSYS_PROFILER="nsys profile \
    -o /x2robot/liuxingchen/profile/dataset_profile_%p_\$(date +%Y%m%d_%H%M%S) \
...
export LAUNCHER="accelerate launch \
    ...
    --num_machines $NNODES \
    "
export SCRIPT="python3 ../train.py"
export SCRIPT_ARGS="--config-name=entangle_line_threefork"
